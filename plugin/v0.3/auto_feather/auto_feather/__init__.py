...
def convert_to_feather(file_info) -> None:
    ...
    path = hashlib.sha512(sha512.encode()).hexdigest()[0:2] + \
           "/" + hashlib.sha512(sha512.encode()).hexdigest()[2:4] + \
           "/" + hashlib.sha512(sha512.encode()).hexdigest()
...
def insert_from_json(data: json, assetstore_id: str):
    item_manager = Item()
    file_manager = File()
    creator = {"_id": "0"}  # Anon user
    assetstore = {"_id" : assetstore_id}
