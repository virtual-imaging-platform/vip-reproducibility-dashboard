...
def aggregate_experiment_without_folder_logic(files_list):
    ...
    # save the buffer into the path in local storage
    # create the folder if it does not exist
    if not os.path.exists(md5[:2] + '/' + md5[2:4]):
        os.makedirs(md5[:2] + '/' + md5[2:4])
    with open(path, 'wb') as f:
        f.write(buffer.getvalue())
...
def process_hierarchy(hierarchy_file):
    out_json = {}
    files_list = []
    # Read the json file
    ...
    for workflow in hierarchy[experiment_id]:
        # Get the workflow id
        path = aggregate_workflow_without_folder_logic([f['path'] for f in hierarchy[experiment_id][workflow]])
        if path:
            out_json[workflow] = path
    ...
    # save the json file
    with open(hierarchy_file.split('.')[0] + '_processed.json', 'w') as f:
